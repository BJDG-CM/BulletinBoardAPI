...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    const tokenResponse = await this.idpService.requestToken(loginDto.code, loginDto.redirectUri);
    const userInfo = await this.idpService.fetchUserInfo(tokenResponse.access_token);
  ...
  private buildLoginResponse(tokenResponse: TokenResponse, userId: string, userInfo: { sub: string; name: string; email?: string }) {
    return {
